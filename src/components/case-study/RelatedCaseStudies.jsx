...
export default function RelatedCaseStudies({ currentStudyId, categories = [] }) {
    const related = useMemo(() => {
        ...
        const shared = others.filter((s) => s.categories?.some((c) => categories.includes(c)));
    ...
    return (
        <section className="space-y-6 pt-6">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
                <div>
                    <span className="text-xs font-semibold text-[var(--text-faint)] block mb-1">
                        Explore more work
                    </span>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {related.map((study) => {
                    ...
                    const thumb = study.website ? heroShot(study.website, 800, 500) : study.images?.[0];
                    ...
                    return (
                        <Link
                            ...
                            className="rounded-2xl overflow-hidden border border-[var(--border)] hover:border-[var(--accent-cyan)]/50 transition-all duration-300 group flex flex-col hover:-translate-y-1 bg-[var(--bg-card)]"
                        >
                            <div className="relative aspect-[16/10] overflow-hidden bg-[var(--bg-surface-soft)]">
                                ...
                                {topStat && (
                                    <span className="absolute bottom-2.5 right-2.5 text-[11px] font-bold px-2.5 py-1 rounded-full bg-black/80 backdrop-blur-md text-white border border-white/10">
                                        {topStat}
                                    </span>
                                )}
                            </div>
                                        ...
                                        {study.categories.slice(0, 2).map((c) => {
                                            ...
                                            return (
                                                <span
                                                    ...
                                                    style={{ color: cMeta.color, background: `${cMeta.color}15` }}
                                                >
                                                    {cMeta.label}
                                                </span>
                                            );
